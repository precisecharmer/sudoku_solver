...
bool SudokuFrame::menu() {
    ...
    cout<<"Before we start solving, You need to input the puzzle into this program"<<endl<<endl;
    cout<<"You can either:-\n";
...
void SudokuFrame::readFrameValues() {
    cout<<endl<<endl<<"Enter the value if value is between (1 to 9)"<<endl;
...
void SudokuFrame::readFrameValuesFile() {
		...
		cout<<endl<<endl<<"Enter the name of the file that contains the Sudoku puzzle."<<endl;
		cout<<"\t --> ";
		cin>>filename;
		...
		for(int rowiter = 0; rowiter < 9; rowiter++) {
            for(int coliter = 0;coliter < 9; coliter++) {
                ...
                if(!(current_value >= 0 && current_value <= 9)) {
                    cout<<endl<<"Value "<<((rowiter * 9) + coliter + 1)<<filename<<" seems to be wrong..Try again with different value"<<endl;
                    exit(EXIT_FAILURE);
                }
